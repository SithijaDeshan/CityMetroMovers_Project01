...
class registerCntrl extends \Classes\register_classes
{
    private $user_fname ;
    private $user_lname ;
    private $user_email ;
    ...
    public function __construct($user_fname,$user_lname,$user_email,$user_contactNo,$user_nic,$user_photo,$user_password,$confirm_user_password){
    ...
    public function registerUser(){
        ...
        if(!$this->phoneValid()){
            ...
            header("Location: ../register.php?error=invalid_phone_number");
            exit();
        }

        if(!$this->nicValid()){
            $_SESSION['message'] = "Invalid NIC number.";
            header("Location: ../register.php?error=invalid_nic_number");
            exit();
        }
        ...
        $this->setUser($this->user_fname,$this->user_lname,$this->user_email,$this->user_contactNo,$this->user_nic,$this->user_photo,$this->user_password);
}

    private function emptyInput(){
        if(empty($this->user_fname) || empty($this->user_lname) || empty($this->user_email) || empty($this->user_contactNo) || empty($this->user_nic) || empty($this->user_photo) || empty($this->user_password) || empty($this->confirm_user_password) ){
            $result = false;
        }else{
            $result = true;
        }
    ...
    private function pwdMatch(){
        if(!filter_var($this->user_password == $this->confirm_user_password)){
            $result = false;
        }else{
            $result = true;
        }
