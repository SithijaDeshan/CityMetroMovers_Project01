...
class register_classes{
    ...
    protected function setUser($user_fname,$user_lname,$user_email,$user_contactNo,$user_nic,$user_photo,$user_password){
        $hasheduser_password = md5($user_password);
        $dbcon = new DBConnector();
        $con = $dbcon->getConnection();
        try {
            $query = "INSERT INTO user (user_fname,user_lname,user_email,user_contactNo,user_nic,user_photo,user_password) VALUES(?,?,?,?,?,?,?)";
            ...
            $pstmt->bindValue(6,$user_photo);
            $pstmt->bindValue(7,$hasheduser_password);
